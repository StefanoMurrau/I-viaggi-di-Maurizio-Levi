...
function richiedi_preventivo_ws1 ($email,$idViaggioKel,$NumAdulti,$NumBambini,$UlterioriRichieste,$datapartenza ){
    ...
    $params->Email 						= $email;
    $params->idViaggioKel 				= $idViaggioKel;
    $params->idDataPartenza 			= $datapartenza;
    $params->idAeroporto                = '';
    $params->businessClass 				= $businessClass;
    $params->numAdulti 					= strval($NumAdulti);
    $params->numBambini 				= strval($NumBambini);
...
function sm_send_mail($to, $subject, $body, $admin_email_headers_from)
{
    ...
    $mail->setFrom("booking-web@villalabianca.com", "I Viaggi di Maurizio Levi");
...
if ($_POST['request_type'] == 'business_request' ) {
    ...
    // $ReqDesc .= " --- Country: " .$PersonCountry = trim($_POST['PersonCountry']) . " --- ";
    // $ReqDesc .= " --- Arrivo: " .$tripstart = trim($_POST['tripstart']) . " --- " ;
    // $ReqDesc .= " --- Partenza: " .$tripend = trim($_POST['tripend']) . " --- " ;
    // $ReqDesc .= " --- Room: " .$roomtype = trim($_POST['roomtype']) . " --- " ;
...
//    $coordxy = $arr_code[0] ."," .$arr_code[1];
    ...
    if ($connessione_ext->connect_errno) {
        ...
        else
        {
            ...
            $sql_insert_ext = "INSERT INTO icnc_crm_person (ID,Name,Surname,Telephone_1,Telephone_2,Email,Web_Site,Birthday,Sex,Role_Organization,Priv_1,Date_Priv_1,Priv_2,Date_Priv_2,Priv_3,Date_Priv_3,Priv_1_Channel,Priv_2_Channel,Priv_3_Channel,ID_FK_Organization,ID_Own_Organization,ID_Own_Person,ID_Group,Created,Created_By,Updated,Updated_By ) VALUES ('$unik','$PersonName','$PersonSurname','$PersonTel','','$PersonEmail','','','','','Y','$timestamp','Y','$timestamp','Y','$timestamp','$timestamp','$timestamp','$timestamp','$organizatione_own_id','$organizatione_own_id','$person_own_id','','$timestamp','$person_own_id','$timestamp','$person_own_id');" ;
        ...
        if($OrgName){
            ...
            else
            {
                ...
                $sql_insert_ext = "INSERT INTO icnc_crm_organization (ID,Name,Type,VAT,CF,Legal_Form,Address,ZIP_Code,City,ID_City,State,ID_State,Nation,ID_Nation,CoordXY,Telephone_1,Telephone_2,Email,Web_Site,Person_Name,Person_Surname,ID_FK_Person,ID_FK_Organization,ID_Own_Organization,ID_Own_Person,ID_Group,Created,Created_By,Updated,Updated_By) VALUES ( '$unik_org','$OrgName','Business','','','','','','$OrgCity','','','','','','','','','','$OrgWebSite','$PersonName','$PersonSurname','$unik','','','$person_own_id','','$timestamp','$person_own_id','$timestamp','$person_own_id' );" ;
            }
        ...
        //echo "Classe: " .$classe;
        $sql_insert_ext = "INSERT INTO icnc_crm_business_request ( ID,Subject,Description,Product,ID_Product,Channel_Type,Channel_Name,ID_FK_Organization,ID_FK_Person,ID_Own_Organization,ID_Own_Person,ID_Group,Created,Created_By,Updated,Updated_By ) VALUES ( '$unik_req','$ReqSubjet','$ReqDesc','$ReqProd','','$ReqType','$ReqName','$organizatione_own_id','$unik','$organizatione_own_id','$person_own_id','','$data_ora_adesso','$person_own_id','$data_ora_adesso','$person_own_id' );" ;
...
//        }
    ...
    else
    {
        ...
        //INVIO EMAIL A BOOKING DICENDO CHE REGISTRAZIONE UTENTE AUTOMATICA NON HA FUNZIONATO -- NON SI PERDE RICHIESTA!
    ...
    if ($reg_ok)
    {
        //entro qui solo se registrazione andata a buon fine
        //echo "tutto bene";
        ...
        $body .= "<br>Per averci chiesto maggiori informazioni da 'I Viaggi di Maurizio Levi.' " ;
        $body .= "<br>";
        ...
        $body_admin = "Richiesta informazioni/iscrizione Newsletter da utente:<br><br>"; 
        $body_admin .= "<br>Nome: " .$PersonName;         
        $body_admin .= "<br>Cognome: " .$PersonSurname; 
        $body_admin .= "<br>Email: " .$PersonEmail;
        $body_admin .= "<br>Tel: " .$PersonTel;
        $body_admin .= "<br>Richiesta: " .$ReqDesc;
        ...
        if( sm_send_mail( $admin_email, $admin_email_subject, $body_admin, $admin_email_headers_from ))
		{
        //echo "inviata admin";
    ...
    {
        $body_admin = "Attenzione!"; 
        $body_admin .= "<br>Un utente ha provato ad iscriversi alla Newsletter de 'I Viaggi di Maurizio Levi.' senza successo:" ;
        $body_admin .= "<br>Nome: " .$PersonName ;
        $body_admin .= "<br>Cognome: " .$PersonSurname ;
        $body_admin .= "<br>Email: " .$PersonEmail ;
